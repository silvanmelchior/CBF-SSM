...
class CBFSSMSingleBW(BaseModel):

    def __init__(self, config):
        super(CBFSSMSingleBW, self).__init__(config)
    ...
    def _backward(self):
        ...
        y2_final = tf.transpose(y2_final.stack(), perm=[1, 0, 2, 3])
        out_dub = tf.tile(tf.expand_dims(self.sample_out, axis=2), [1, 1, samples, 1])
        self.y_tilde = tf.concat((out_dub, y2_final), axis=3)
    ...
    def _backward_body(self, u, y, y2, p, t, h):
        ...
        u_t = u.read(t)
        y_t = y.read(t)
        hidden = tf.cond(tf.equal(tf.mod(t + 1, recog_len), 0),
